# Social Network Theme - Webpack5 + HTML + SASS + Vanilla JS

Bu proje yapısında HTML, SASS(SCSS) ve Vanilla JS ile front-end uygulama geliştirebilirsiniz. Buradaki asıl amaç SCSS kullanarak örnek bir template oluşturabilmektir. SASS tarafındaki önemli konu başlıklarını ele alan bir örnek incelemek istiyorsanız bu uygulama tam size göre demektir. Klasörü yapısını abartmamak için `7-1 SASS` Dosya Mimarisine uygun hazırlamadım. Zaten küçük çaplı uygulamalarda buna gerek yok.

## Yapılandırma Kapsamı
1. HTML (HtmlWebpackPlugin)
2. Vanilla JS (Babel)
3. SASS Basic Folder Structure
4. Normalize.css
